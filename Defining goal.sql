...
SELECT
	*,
	cs.total_revenue*100/ts.sales AS contribution_percent
...
--count of delayed or timely shipped orders

WITH order_status AS(
	SELECT
		CASE 
			WHEN shipped_date IS NULL THEN 'Not shipped'
			WHEN shipped_date <= required_date THEN 'Timely shipped'
...
"monthly revenue with month number"
SELECT
...
"monthly revenue with months as 'mmm' format"
SELECT
    TO_CHAR(o.order_date, 'Mon') AS months,
    EXTRACT(YEAR FROM o.order_date) AS years,
    SUM(od.unit_price * od.quantity * (1 - od.discount)) AS total_revenue
FROM orders o
JOIN order_details od ON o.order_id = od.order_id
GROUP BY 
    ...
    EXTRACT(YEAR FROM o.order_date)
ORDER BY years, MIN(o.order_date);
