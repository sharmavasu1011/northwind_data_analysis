...
2. Products that generate high revenue but low quantity (premium items) 
...
3. Products with high quantity but low revenue (low margin candidates) 
...
4. Slow-moving products (inventory insight)
...
2. Top repeat customers vs one-time buyers 
...
5. Customers who stopped ordering (churn detection)
...
6. Revenue per country normalized by number of customers 
...
7. Expansion opportunities (countries with few customers but high spend)
...
8. Contribution of top 20% customers (Pareto analysis)
...
3. Average order value handled by each employee 
...
4. Monthly sales trends (not just yearly) 
...
5. Best performing month/quarter 

WITH monthly_sales AS (
	SELECT 
		EXTRACT(MONTH FR
		OM o.order_date) AS month,
		ROUND(SUM(od.unit_price * od.quantity * (1-od.discount)),2) AS total_sales
	FROM orders o 
	JOIN order_details od ON o.order_id = od.order_id
	GROUP BY month
...
6. Revenue with vs without discount 
...
8. Are discounts actually increasing sales or just reducing profit?
...
3. Average shipping delay (shipped_date - order_date) 
...
5. Shipper performance comparison
...
7. Distribution of order sizes (small vs bulk orders)
